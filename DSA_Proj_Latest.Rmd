...
A1 <- Arima(seasadj_wiki, order=c(2,1,2), seasonal = list(order = c(2,0,3), period = 7))
A2 <- Arima(seasadj_wiki, order=c(2,1,3), seasonal = list(order = c(2,0,2), period = 7))
A3 <- Arima(seasadj_wiki, order=c(2,1,3), seasonal = list(order = c(2,0,3), period = 7))
A4 <- Arima(seasadj_wiki, order=c(2,1,3), seasonal = list(order = c(2,1,1), period = 7))
A5 <- Arima(seasadj_wiki, order=c(3,1,3), seasonal = list(order = c(2,1,1), period = 7))
A6 <- Arima(seasadj_wiki, order=c(3,1,3), seasonal = list(order = c(2,1,2), period = 7))
A7 <- Arima(seasadj_wiki, order=c(4,1,3), seasonal = list(order = c(2,1,2), period = 7))
...
f1 = stlm(wiki_daily_split$train, modelfunction = Arima, order=c(2,1,2),
...
f7 = stlm(wiki_daily_split$train, modelfunction = Arima, order=c(4,1,3), 
         seasonal = list(order = c(2,1,2), period = 7), robust = T) 
...
training_index <- seq(1,floor(0.8*length(views)))
...
model = keras_model_sequential() %>%   
   layer_simple_rnn(units=max_len, input_shape=c(max_len, 1), activation="relu") %>%  
...
model <-  keras_model_sequential() %>%   
   layer_lstm(units=max_len, input_shape=c(max_len, 1), activation="relu") %>%  
   #layer_dense(units=6, activation = "relu") %>%  
   layer_dense(units=1, activation = "linear")
