...
daily_data = daily_data %>% mutate(day_week = as.factor(wday(ymd(daily_data$date), label = T)))
...
ggplot(daily_data[train_size:length(daily_data),], aes(x = day_week, y = views))+geom_boxplot()
...
# Benchmark Models
plot_info(b1, "Naive Model")
plot_info(b2, "Drift Model")
plot_info(b3, "ETS Model")
# Best ARIMA Models
plot_info(f7, "ARIMA (3,1,3)(2,1,3)[7] Model")

# Prophet

x_axes = seq(1:(length(prophet_predict$yhat)))
plot(x_axes, prophet_test$y, type="l", col="red", lwd=2)
lines(x_axes, prophet_predict$yhat, col="blue",lwd=2)
legend("topleft", legend=c("y-original", "y-predicted"), col=c("red", "blue"), lty=1,cex=0.8)
plot(model, prophet_forecast)+ggtitle("Prophet Model")

# Prophet vs STL Decomp
