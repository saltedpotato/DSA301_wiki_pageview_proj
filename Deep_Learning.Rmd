...
lstmforecast = function(yvariable, forecasthorizon){
  ...
  lstm_model %>% fit(x = x_train_arr, y = y_train_arr, batch_size = 1, epochs = 20,shuffle = FALSE, validation_split = 0.2, callbacks = list(callback_early_stopping(monitor = 'val_loss', patience = 3, verbose = 1, restore_best_weights = T)) )    
...
train_trend <- mstl(msts_train)[,2]
arima_seasadj <- auto.arima(train_trend, seasonal = F)

y_pred_trend <- forecast(arima_seasadj, h = forecasthorizon)
y_pred_trend <- y_pred_trend$mean 

var7 = seasonal(mstl(msts_wiki_daily))[,1]
...
var365 = seasonal(mstl(msts_wiki_daily))[,2]
...
y_pred <- y_pred_trend + y_pred_seas7 + y_pred_seas365
