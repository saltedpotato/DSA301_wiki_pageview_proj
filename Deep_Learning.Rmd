...
lstmforecast = function(yvariable, forecasthorizon){
  ...
  lstm_model %>% fit(x = x_train_arr, y = y_train_arr, batch_size = 1, epochs = 20,shuffle = FALSE, callbacks = list(callback_early_stopping(monitor = 'val_loss', patience = 6, verbose = 1, restore_best_weights = T)) )    
...
y_pred <- forecastedseries[(length(forecastedseries) - 482 + 1): length(forecastedseries)]
y_test <- var[(length(var) - 482 + 1):length(var)]
