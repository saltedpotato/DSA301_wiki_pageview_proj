...
getNN_accuracy = function(data, p = 35, P= 5, xregterm = NULL, xregterm_forecasted= NULL){
    ...
    final_forecast = as.vector(seasadj_forecast$mean) + as.vector(seas7$mean) + as.vector(seas365$mean)
    # print(checkresiduals(final_forecast))
...
results = c()
for (week in 1:6){
  model_rmse = getNN_accuracy(data = seasadj_wiki, p = week*7)[,2]
  results = append(results, model_rmse)
}

results
plot(results, type = "l", xlab = "Weeks of non-seasonal lags used for input", ylab = "Validation RMSE", col = "blue", title = "")
...
NN_diff_1 = nnetar(seasadj_wiki_diff, p = 35, P = 5)
...
# 1st variable: Differenced information forecasted using a feedforward neural network. 2nd Variable: 2020 
exogenous_var = cbind(seasadj_wiki_diff, train2020_dummy)
# exogenous_var_val = cbind(NN_diff_pred,test2020_dummy)
...
# Building the model
# getNN_accuracy(data = tail(seasadj_wiki, length(exogenous_var[,1])), xregterm = exogenous_var, xregterm_forecasted = exogenous_var_val_alt)
