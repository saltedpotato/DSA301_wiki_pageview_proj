...
Since m7 has an AIC close to m6, we will consider it as well. 

From the checkresidueals test, m1 (auto-arima) and m6 both pass the Ljung-box test with p-value of 0.07454 and 0.08318 respectively at 10% sig. level.
m7 also pass the Ljung-box test with p-value of 0.04768 at 5% sig. level.

Hence at 10% sig level, all 3 models have residuals that resemble white noise, and at 5% sig level, only m7 have residuals resembling resembling noise.
...
accuracy(forecast(m7,h = 15*12), latest_data_ts)

autoplot(forecast(m7,h = 15*12), PI = F)
autoplot(forecast(m7,h = 15*12), PI = T)

```

From the accuracy tests, m1 attained the best RMSE score of 5.323888 in the train set. However, m7 performed the best in the test set with a RMSE score of 10.308077. Hence, m7 is the best model to use for prediction.

Q1 (g)
Since the predications of m7 is reasonably accurate, the general predictability of the trend, seasonality, and heteroskedastic attributes of the time series makes for a more accurate forecast. With such long and stable patterns in the data, predictions up to a decade would not be unseasonable. Afterwards, predictions may no longer be accurate, and should no longer be used.
